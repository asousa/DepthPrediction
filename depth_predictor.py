...
class DepthPredictor():
   ...
   def load(self,model_file=None,pretrained_file=None, meanfile=None, image_dims=(227,227)):
      ...
      #   self.unaryCNN.set_mean(self.unaryCNN.inputs[0], meanfile)
   ...
   def predict(self, input_img, no_superpixels=400, true_depths=None):
      ...
      for ix, in_ in enumerate(segs):
         input_[ix] = caffe.io.resize_image(in_.transpose(2,1,0), self.unaryCNN.image_dims).astype('uint8')
      ...
      # Regression output
      return image_handling.apply_depths(predictions, mask)
